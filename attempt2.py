...
X = X.iloc[:, X.columns!='Name']
X = X.iloc[:, X.columns!='Ticket']
X = X.iloc[:, X.columns!='Cabin']
X = X.iloc[:, X.columns!='Parch']
X = X.iloc[:, X.columns!='SibSp']
X = X.iloc[:, X.columns!='Fare']
...
classifier.add(Dense(output_dim = 12, init = 'uniform', activation = 'relu', input_dim = 4))

# Adding the second hidden layer
classifier.add(Dense(output_dim = 15, init = 'uniform', activation = 'sigmoid'))
